...
def query_grid(im, query_x, query_y, method='bilinear'):
    ...
    Uses
        - Bilinear interpolation that allows you to break big matrices into patches and work locally; or
        - Bivariate spline interpolation that fits a global spline (so memory-intensive) and shows
          global effects.

    Args:
        im (numpy.ndarray): H-by-W or H-by-W-by-C rectangular grid of data.
            Each of C channels is interpolated independently.
        query_x (array_like): x coordinates of the queried rectangle, e.g., ``range(10)`` for a 10-by-10
            grid (hence, this should *not* be generated by :func:`numpy.meshgrid` or similar functions).
