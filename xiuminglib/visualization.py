...
def pyplot_wrapper(*args,
    ...
    Args:
        ...
        *_rotation (float, optional): Tick rotation in degrees.
        ...
        outpath (str, optional): Path to which the visualization is saved to.
            ``None`` means ``os.path.join(xlib.constants['dir_tmp'], 'pyplot_wrapper.png')``.
    ...
    if outpath is None:
        outpath = join(xlib.constants['dir_tmp'], 'plot.png')
...
def scatter_on_image(im, pts, size=2, bgr=(0, 0, 255), outpath=None):
    ...
    if outpath is None:
        outpath = join(xlib.constants['dir_tmp'], 'scatter_on_image.png')
...
def matrix_as_image(arr, outpath=None, gamma=None):
    ...
    Args:
        ...
        outpath (str, optional): Where to visualize the result to. ``None`` means
            ``os.path.join(xlib.constants['dir_tmp'], 'matrix_as_image.png')``.
    ...
    if outpath is None:
        outpath = join(xlib.constants['dir_tmp'], 'matrix_as_image.png')
    ...
    if gamma is not None:
        im = xlib.image_processing.gamma_correct(im, gamma)
...
def matrix_as_heatmap_complex(*args, **kwargs):
    ...
    if outpath is None:
        outpath = join(xlib.constants['dir_tmp'], 'matrix_as_heatmap_complex.png')
...
def uv_on_texmap(u, v, texmap, ft=None, outpath=None, figtitle=None):
    ...
    Args:
        ...
        outpath (str, optional): Path to which the visualization is saved to.
            ``None`` means ``os.path.join(xlib.constants['dir_tmp'], 'uv_on_texmap.png')``.
        figtitle (str, optional): Figure title.
    ...
    if outpath is None:
        outpath = join(xlib.constants['dir_tmp'], 'uv_on_texmap.png')
    ...
    Args:
        ...
        *_rotation (float, optional): Tick rotation in degrees.
        ...
        outpath (str, optional): Path to which the visualization is saved to. Should end with
            ...
            ``None`` means ``os.path.join(xlib.constants['dir_tmp'], 'axes3d_wrapper.png')``.
...
def ptcld_as_isosurf(pts, out_obj, res=128, center=False):
    ...
    # Point cloud to TDF
    tdf = xlib.geometry.ptcld2tdf(pts, res=res, center=center)
